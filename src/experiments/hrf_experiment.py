...
class HrfExperimentArgs(
    BaseExperimentArgs, AdamArgs, StepLRArgs, HrfDatasetArgs, AutoSamModelArgs
):
...
class HrfExperiment(BaseExperiment):
    def __init__(self, config: dict[str, Any], yaml_config: YamlConfigModel):
        self.config = HrfExperimentArgs(**config)
        self.retina_data = HrfDataset(config=self.config, yaml_config=yaml_config)
        super().__init__(config, yaml_config)

    def get_name(self) -> str:
        return "hrf_experiment"
